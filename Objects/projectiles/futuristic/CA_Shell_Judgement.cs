 ...
 /// </summary>
    ...
    public class CA_OrbitalStrike : IAITankDanger
    {
       // private Vector2 _oldPosition;
        ...
        /// <param name="owner"> the tank that fired it</param>
        ...
        internal void Update()
        {
            ...
            if (Server.NetManager != null || !Client.IsConnected())
            {
                ...
                if(!Firing && Ani_ObliterateScale>0f)
                {
                    ...
                    {
                        ring.Alpha -= 0.1f * TankGame.DeltaTime;

                        GeometryUtils.Add(ref ring.Scale, (0.03f + WarningForgiveness*0.035f) * TankGame.DeltaTime);
                        if (ring.Alpha <= 0)
                            ring.Destroy();
                    };
                    ...
                    {
                        ring2.Alpha -= 0.1f * TankGame.DeltaTime;

                        GeometryUtils.Add(ref ring2.Scale, (0.06f + WarningForgiveness * 0.035f) * TankGame.DeltaTime);
                        if (ring2.Alpha <= 0)
                            ring2.Destroy();
                    };
                }
                ...
                if ((Math.Floor(LifeTime) % 60f) % 20f == 0 && LifeTime < WarningTime)
                {
                    ...
                    ring.UniqueBehavior = (a) =>
                    {
                        ...
                        GeometryUtils.Add(ref ring.Scale, (0.01f + WarningForgiveness * 0.035f) * TankGame.DeltaTime);
                        if (ring.Alpha <= 0)
                            ring.Destroy();
                    };
                }
    ...
    public void CheckCollisions_DeathBeam()
        {
            ...
            for (var i = 0; i < GameHandler.AllTanks.Length; i++)
            {
                ...
                tank.Damage(new TankHurtContextOther(IsPlayerSourced,TankHurtContextOther.HurtContext.FromIngame,"JUDGED",this));
            }
