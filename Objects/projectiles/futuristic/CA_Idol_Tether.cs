    ...
    public class CA_Idol_Tether
    {
        ...
        public CA_Idol_Tether(Tank host, Tank target)
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                float h = 5f;
                Vector3 path = (Vector2.UnitY.RotatedByRadians(a)*target.CollisionCircle.Radius*1.1f).ExpandZ()+ Vector3.UnitY * h;
                var p = GameHandler.Particles.MakeParticle(path, TankGame.WhitePixel);
        ...
        internal void Update()
        {
            ...
            for (int i = 0; i < vfx_shield.Count; i++)
            {
                ...
                float h = 5f;
                Vector3 path = (Vector2.UnitY.RotatedByRadians(a) * bindTarget.CollisionCircle.Radius * 1.1f).ExpandZ() + Vector3.UnitY * h;
