...
namespace CobaltsArmada
{
    ...
    public class BossBar
    {
        ...
        public void Render(SpriteBatch sb, Vector2 position, Vector2 scale, Anchor aligning, Color emptyColor, Color fillColor)
        {
            ...
            Hp_Ticked_Ouched -= RuntimeData.DeltaTime;
            if(Hp_Ticked_Ouched<=0 && Hp_Ticked> BossHP)
            {
                Hp_Ticked -= 0.01f* BossHPMax * RuntimeData.DeltaTime;
           ...
           TankGame.SpriteRenderer.DrawString(FontGlobals.RebirthFont, _Title, finalpos - (Vector2.UnitY * 20).ToResolution(), Color.White, new Vector2(0.6f).ToResolution(), 0f, FontGlobals.RebirthFont.MeasureString(_Title)/2f,0f);
